...
const items = computed(() => [
  ...
  { imgSrc: czechKorunaImage, altText: 'czech koruna cash', description: content.value.czechKorunaDescription },
])
    ...
    <div>
      ...
      <div class="flex justify-evenly">
        <div v-for="(item, index) in items" :key="index" class="flex flex-col items-center justify-center">
          ...
          <p class="text-lg text-center font-semibold text-monero-grey m-3">{{ item.description }}</p>
        </div>
      </div>
    </div>
